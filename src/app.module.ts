...
@Module({
  imports: [
    ...
    ConfigModule.forRoot({
      isGlobal: true,
      load: [appConfig, databaseConfig, redisConfig, bullmqConfig, geminiConfig, scraperConfig, ragConfig],
    }),
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        entities: [Website, Page, Chunk, Conversation, Message, Escalation, User],
        synchronize: config.get<string>('app.env') === 'development',
        logging: config.get<string>('app.env') === 'development' ? ['error'] : false,
      }),
    }),
